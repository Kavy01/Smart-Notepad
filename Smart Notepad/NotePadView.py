...
class Notepad:
    def __init__(self):
        ...
        cur_time2 = datetime.datetime.now()
        temp = []
        temp.append(00)
        temp.append(00)
        temp.append(cur_time2.second)

        t1 = Thread(target=self.changed,args = temp)
        t1.start()
    ...
    def check_db_status(self):
        ...
        if self.db_status:
            self.set_files = self.db_controller.load_files_from_db()
        else:
    ...
    def exit_func(self, event=None):
        try:
            if self.text_changed==True:
                ans = messagebox.askokcancel("Saving File","Do you want to save the file?")
                if ans==True:
    ...
    def open_file(self, event=None):
        try:
            ...
            result = self.db_controller.is_secure_file(self.base)
            if result:
                pwd = self.get_file_pwd()
                if pwd==self.db_controller.get_file_pwd():
    ...
    def changed(self,h2,m2,sec, event=None):
        while True:
            cur_time = datetime.datetime.now()
            h1,m1,s1 = cur_time.hour,cur_time.minute,cur_time.second
            s2 = abs(s1 - sec)
            if s1 <= sec:
                s2 = 60-abs(s1-sec)
            if m2==60:
            ...
            if self.text_editor.edit_modified():
                ...
                character = len(self.text_editor.get(1.0,'end-1c'))
                self.status_bar.config(text=f'Words: {words} Characters: {character} Time: {str(h1)+":"+str(m1)+":"+str(s1)} App Running Time: {str(h2)+":"+str(m2-1)+":"+str(sec)}')
            else:
                words = len(self.text_editor.get(1.0, 'end-1c').split())
                character = len(self.text_editor.get(1.0, 'end-1c'))
                self.status_bar.config(text=f'Words: {words} Characters: {character} Time: {str(h1) + ":" + str(m1) + ":" + str(s1)} App Running Time: {str(h2)+":"+str(m2-1)+":"+str(s2)}')
    ...
    def find(self):
        ...
        if word:
            ...
            while True:
                ...
                self.text_editor.tag_config.configure('match',background='yellow',foreground='red')
                matches+=1
                start_pos=end_pos
    ...
    def say_something(self):
        try:
            self.voice = self.file_controller.take_query()
            if self.voice == "":
                messagebox.showinfo("Say Again.","Speech not recognized!!")
            elif self.voice.lower() == 'hey mojo open file':
                self.open_file()
            elif self.voice.lower() == 'hey mojo save file':
                self.save_file()
            elif self.voice.lower() == 'hey mojo new file':
                self.new_file()
            elif self.voice.lower() == 'hey mojo file save':
                self.save_as()
            elif self.voice.lower() == 'hey mojo close app':
                self.exit_func()
            elif self.voice.lower() == 'hey mojo text bold':
                self.change_bold()
            elif self.voice.lower() == 'hey mojo text italic':
                self.change_italic()
            elif self.voice.lower() == 'hey mojo search':
                self.find_func()
            elif self.voice.lower() == 'hey mojo hide statusbar':
                self.hide_statusbar()
            elif self.voice.lower() == 'hey mojo hide toolbar':
                self.hide_toolbar()
        except RequestError as req:
            messagebox.showerror("No Internet","Please check your internet connection!")
        except WaitTimeoutError as req:
            messagebox.showerror("Timeover","Time limit exceeded!")
        except UnknownValueError as req:
    ...
    def hide_statusbar(self):
        if self.show_statusbar:
            self.show_statusbar.pack_forget()
    ...
    def change_theme(self):
        self.theme_name = self.theme_choice.get()
        self.theme = self.color_dict.get(theme_name)
        bg_color,fg_color = self.theme
        self.text_editor.config(background=bg_color,fg=fg_color)

    def change_font(self, event=None):
        self.font_name = self.font_family.get()
        self.text_editor.config(font=(self.font_name,self.current_font_size))
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            for values in self.db_controller.db_model.file_dict.keys():
                self.f_name = values
                file_owner = self.db_controller.db_model.file_dict.get(self.f_name)[1]
    ...
    def get_file_pwd(self):
        str = simpledialog.askstring("Password","Enter Your Password: ",show="*")
        return str
    def run(self):
        self.root.mainloop()
